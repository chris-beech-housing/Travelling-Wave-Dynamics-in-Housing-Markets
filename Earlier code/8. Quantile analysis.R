...
monthly_median |>
  ...
  layout(
    # title = list(
    #   text = "Moving average of the annual log return of the monthly median, by quantile", # adjust the title as necessary
...
# Plot rank
legend_numbers <- seq(30, 300, by = 50)
...
monthly_median |>
  ...
  layout(
    # title = list(
    #   text = "Rank of the moving average of the annual return of the monthly median, by quantile", # adjust the title as necessary
    #   font = list(family = "Helvetica", size = 20),
    #   x = 0.063
    # ),
    # margin = list(t = 33),
...
cluster_data_scaled <-
  ...
  arrange(quantile) |>
  filter(!is.na(log_return)) |>
  select(quantile, Date, log_return) |>
  pivot_wider(names_from = Date, values_from = log_return) |>
  column_to_rownames("quantile") |>
  as.matrix() |>
  scale()
...
kmeans_result <- kmeans(cluster_data_scaled, centers = 4) # use the whole data set to preserve all original features
...
# Prepare and plot
pca_scores <- as_tibble(pca_result$x, rownames = "quantile")
...
pca_scores |>
  ...
  geom_text_repel(
    data = pca_scores,
    aes(x = PC2, y = PC1, label = quantile),
    size = 3
  ) +
  coord_cartesian(clip = 'off') +
  scale_y_reverse() +
  # labs(title = "Principal Component Analysis K-Means Clustering, by quantile") +
  theme_minimal() +
...
spatial_edges <-
  monthly_median |>
  select(quantile, Date, Rank) |>
  filter(!is.na(Rank)) |>
  arrange(Date, Rank) |>
  group_by(Date) |>
  mutate(to = lead(quantile)) |>
  ungroup() |>
  filter(!is.na(to)) |>
  mutate(type = "spatial") |>
  select(from = quantile, to, type)
...
row_ids <- as.numeric(rownames(adj_matrix))
col_ids <- as.numeric(colnames(adj_matrix))

adj_matrix_long <-
  data.frame(
    x = rep(col_ids, each = length(row_ids)),
    y = rep(row_ids, times = length(col_ids)),
...
adj_matrix_long |>
  ...
  layout(
    # title = list(
    #   text = "Spatiotemporal adjacency matrix of the ranks, by quantile",
    #   font = list(family = "Helvetica", size = 20),
    #   x = 0.0475
    # ),
    # margin = list(t = 33),
