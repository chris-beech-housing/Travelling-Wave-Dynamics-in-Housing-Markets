...
price_paid_data <-
  read_csv("Data/pp-subset.csv") # or pp-psqm, or pp-repeat
...
price_paid_data |>
  select(
    ...
    Region,
    `Property Type`,
    Price_inc_Stamp_Duty
  ) |>
  write_csv("Data/pp-stampduty.csv")

# Or for the psqm data
price_paid_data |>
  mutate(Price_inc_Stamp_Duty = Price_inc_Stamp_Duty / tfarea) |>
  select(
    `Date of Transfer`,
    `Local Authority`,
    Region,
